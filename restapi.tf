...
resource "aws_api_gateway_authorizer" "restapi" {
  ...
  authorizer_uri = try(each.value.auth, "") == "CUSTOM" ? "arn:aws:apigateway:${local.region_name}:lambda:path/2015-03-31/functions/${aws_lambda_function.function[each.value.lambda].arn}/invocations" : null
